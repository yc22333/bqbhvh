...
(function() {
  ...
  /* ── 获取API基础路径 ── */
  function getApiBase() {
    var isLocal = window.location.protocol === 'file:' || window.location.hostname === '' || window.location.hostname === '127.0.0.1' || window.location.hostname === 'localhost';
    return isLocal ? '/netlify/functions' : '/.netlify/functions';
  }
  ...
  function loadItemsFromCloud() {
    ...
    var url = getApiBase() + '/items?limit=5000';
  ...
  function clearLogsFromCloud(adminPass) {
    var url = getApiBase() + '/logs?admin_key=' + encodeURIComponent(adminPass);
  ...
  /* ══════════════════════════════════════════════════════════════
     ③ 游戏存档（原 youxi3.js localStorage）
     ══════════════════════════════════════════════════════════════ */

  /* ── 生成玩家标识（基于浏览器指纹简单实现） ── */
  function getPlayerId() {
    var pid = localStorage.getItem('bqb_player_id');
    if (!pid) {
      pid = 'player_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
      localStorage.setItem('bqb_player_id', pid);
    }
    return pid;
  }

  /* ── 从云端加载游戏存档 ── */
  function loadGameSaveFromCloud(gameId) {
    var playerId = getPlayerId();
    var url = getApiBase() + '/game-saves?player_id=' + encodeURIComponent(playerId);
    return fetch(url)
      .then(function(res) { return res.json(); })
      .then(function(result) {
        if (result.ret === 0 && result.data) {
          return result.data.save_data || null;
        }
        return null;
      })
  ...
  /* ── 保存游戏存档到云端 ── */
  function saveGameSaveToCloud(gameId, saveData) {
    var playerId = getPlayerId();
    var url = getApiBase() + '/game-saves?player_id=' + encodeURIComponent(playerId);
    return fetch(url, {
      method: 'POST',
  ...
  function init() {
    ...
    Mode.Debug.log('CloudData', '☁️ 物品数据: 从Supabase(game_items)读取');
    Mode.Debug.log('CloudData', '☁️ 系统日志: 从Supabase(system_logs)读写');
    Mode.Debug.log('CloudData', '☁️ 游戏存档: 从Supabase(game_saves)读写');
  ...
  Mode.CloudData = {
    ...
    // ── 游戏存档 ──
    loadGameSave: loadGameSaveFromCloud,
    saveGameSave: saveGameSaveToCloud,
