...
const GuanYu = (() => {
  ...
  function getCacheManagerApiUrl() {
    ...
    return isLocal ? '/netlify/functions/cache-manager' : '/.netlify/functions/cache-manager';
  }
  ...
  async function refreshSingleCache(cacheType) {
    ...
    try {
      ...
      if (data.ret === 0) {
        ...
        // 通知前端重新加载该缓存数据（用户下次访问从Supabase获取）
  ...
  function doCacheLogin() {
    ...
    var apiUrl = '/.netlify/functions/orders';
    if (window.location.protocol === 'file:' || window.location.hostname === '' || window.location.hostname === '127.0.0.1' || window.location.hostname === 'localhost') {
