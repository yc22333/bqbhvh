...
const Zuo = (() => {
  ...
  function cleanupGameResources() {
    ...
    // 只停止挖宝游戏定时器（Game3），不停止气泡游戏（Game1 是框架）
    if (window.Game3 && Game3.stop) Game3.stop();
    
    Mode.Debug.log('Zuo', '游戏资源已清理');
  }

  /* ── 切换主内容区页面 ── */
  function switchMainContent(pageId) {
    // 如果不是切换到游戏页面，清理游戏资源
    if (pageId !== 'laoquyouxi') {
    ...
    var hafubihuishouSection = document.getElementById('hafubihuishouSection');
    var maiwuziSection = document.getElementById('maiwuziSection');
    var paodaoSection = document.getElementById('paodaoSection');
    var laoquyouxiSection = document.getElementById('laoquyouxiSection');
    var guanyuSection = document.getElementById('guanyuSection');
    var leidaSection = document.getElementById('leidaSection');
    ...
    if (hafubihuishouSection) hafubihuishouSection.classList.remove('page-active');
    if (maiwuziSection) maiwuziSection.classList.remove('page-active');
    if (paodaoSection) paodaoSection.classList.remove('page-active');
    if (laoquyouxiSection) laoquyouxiSection.classList.remove('page-active');
    if (guanyuSection) guanyuSection.classList.remove('page-active');
    if (leidaSection) leidaSection.classList.remove('page-active');
    ...
    } else if (pageId === 'hafubihuishou' && hafubihuishouSection) {
      hafubihuishouSection.classList.add('page-active');
    ...
    } else if (pageId === 'laoquyouxi' && laoquyouxiSection) {
      laoquyouxiSection.classList.add('page-active');
      // 确保游戏内容被正确渲染
      setTimeout(function() {
        if (window.Game1 && Game1.init) Game1.init();
        if (window.Game3 && Game3.init) Game3.init();
      }, 50);
    ...
    } else if (pageId === 'leida' && leidaSection) {
      leidaSection.classList.add('page-active');
      // 触发雷达页面渲染
      var leidaModule = window.Leida || Leida;
      if (leidaModule && leidaModule.render) {
        leidaModule.render();
      } else {
        Mode.Debug.warn('Zuo', 'Leida 模块未就绪');
      }
    }
  }
  ...
  function initPages() {
    ...
    panel.addEventListener('click', function (e) {
      ...
      if (item && item.dataset.page) {
        ...
        // 处理主内容区页面切换
        if (pageId === 'shouye' || pageId === 'shushukazhanbei' || pageId === 'awmzidan' || pageId === 'hafubihuishou' || pageId === 'maiwuzi' || pageId === 'paodao' || pageId === 'laoquyouxi' || pageId === 'guanyu' || pageId === 'leida') {
  ...
  function showDropdown() {
    ...
    if (dropdownNav) {
      dropdownNav.innerHTML =
        ...
        '</div>' +
        /* ★ 哈夫币回收已暂时隐藏，恢复时取消注释即可 ★
        '<div class="zuo-item" data-page="hafubihuishou">' +
          '<span class="zuo-label">哈夫币回收</span>' +
        '</div>' +
        ★ */
        ...
        '</div>' +
        '<div class="zuo-item" data-page="laoquyouxi">' +
          '<span class="zuo-label">牢区小游戏</span>' +
        '</div>' +
        '<div class="zuo-item" data-page="guanyu">' +
          '<span class="zuo-label">关于</span>' +
        '</div>' +
        '<div class="zuo-item" data-page="leida">' +
          '<span class="zuo-label">雷达</span>' +
        '</div>';
    }
