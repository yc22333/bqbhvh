  ...
  function initAwmDonations() {
    ...
    function renderAddForm() {
      ...
      return '<div class="admin-add-form admin-panel" style="margin-top:0;margin-bottom:12px;">' +
        ...
        '<button id="donationTypeZiji" class="admin-input" style="padding:8px;cursor:pointer;text-align:center;background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.1);font-weight:400;">自己跟车<br><span style="font-size:11px;font-weight:400;">(1.25/发)</span></button>' +
        '</div></div>' +
        ...
        if (dashangBtn && zijiBtn) {
          function setType(type) {
            if (type === 'dashang') {
              ...
              zijiBtn.innerHTML = '自己跟车<br><span style="font-size:11px;font-weight:400;">(1.25/发)</span>';
          ...
          tableContainer.addEventListener('input', function(e) {
            if (e.target.classList.contains('edit-bullets') || e.target.classList.contains('edit-price')) {
              ...
              if (e.target === bulletsInput || e.target === typeSelect || e.target === priceInput) {
                ...
                var price = isShanghao ? qty * 1.30 : qty * 1.25;
  ...
  function initAmountCalc() {
    ...
    function calcPanel(cfg) {
      ...
      if (cfg.id === 'buy') {
        ...
        if (unitPrice == null) unitPrice = 1.25; // 兜底
      } else {
