  ...
  function renderAdminPanel(container) {
    ...
    document.getElementById('ordersTableContainer').addEventListener('input', function(e) {
      if (e.target.classList.contains('edit-bullets') || e.target.classList.contains('edit-price')) {
        ...
        var pricePerBullet = isDashang ? 0.70 : 0.65;
    ...
    if (dashangBtn && zijiBtn) {
      function setOrderType(type) {
        if (type === 'dashang') {
          ...
          zijiBtn.innerHTML = '自己跟车<br><span style="font-size:11px;font-weight:400;">(0.65/发)</span>';
          ...
          dashangBtn.innerHTML = '打手上号<br><span style="font-size:11px;font-weight:400;">(0.70/发)</span>';
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
          ...
          <div style="display:flex;flex-direction:column;">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
              ...
              <button id="ordersTypeZiji" class="admin-input" style="padding:8px;cursor:pointer;text-align:center;background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.1);font-weight:400;">自己跟车<br><span style="font-size:11px;font-weight:400;">(0.65/发)</span></button>
            </div>
          </div>
          <!-- 右列：子弹数量 + 总价 -->
      ...
      .then(function(result) {
        if (result.ret === 0 && result.data) {
          ...
          if (msg.indexOf('Could not find the table') !== -1 || 
            ...
            Mode.showToast('️ 数据库未初始化，请检查服务器端SQLite表结构', 'warn');
          } else {
