/* ============================================================
   ...
   ────────────────────────────────────────────
   基础路径: /api/api-reader
   数据源: 本地 SQLite 数据库（通过 Express 服务器代理）
   Token 注入: 无需 Token（本地服务器直连）
   ────────────────────────────────────────────
   ① 卡战备（3h 缓存）
...
(function() {
  ...
  // 价格/元数据/军需处/钥匙卡/子弹包/筛选项 全部从本地SQLite game_items 表读取
  var CONFIG = {
  ...
  /* ── 从本地服务器读取缓存数据（唯一数据源）── */
  function sqliteFetch(endpoint, logTag) {
    var url = 'api/api-reader.php?endpoint=' + encodeURIComponent(endpoint);
    Mode.Debug.log('SjzCache', '☁️ ' + (logTag || endpoint) + ' 从本地服务器读取...');
    ...
    return fetch(url, {
      ...
      if (result.ret === 0 && result.data) {
        ...
        Mode.Debug.warn('SjzApi', '⚠️ ' + (logTag || endpoint) + ' 无缓存数据: ' + (result.msg || ''));
        return null;
      }
    }).catch(function(err) {
  ...
  /* ── 卡战备 V3（从本地服务器读取，每档位单独API调用，已过滤兑换组+排序取前10）── */
  function fetchBattleV3ZB() {
    // 从本地服务器读取（通过api-reader）
    return sqliteFetch('card_zhanbei', 'V3 战备').then(function(data) {
      if (data) {
  ...
  /* ── 每日地图密码（从本地服务器读取，确保多设备一致）── */
  function fetchMapPwd() {
    // 从本地服务器读取（通过api-reader）
    return sqliteFetch('map_pwd', '每日密码').then(function(data) {
      if (data) {
        return data;
      }
      // 如果本地服务器无数据，返回空对象
  ...
  function init() {
    ...
    Mode.Debug.log('SjzApi', '🛡️  卡战备V3:   Mode.SjzApi.battleV4ZB()  — 本地服务器');
    Mode.Debug.log('SjzApi', '🔧  今日制造Pro: Mode.SjzApi.manufacture() — 本地服务器');
    Mode.Debug.log('SjzApi', '🔑  每日密码:    Mode.SjzApi.mapPwd()      — 本地服务器');
    Mode.Debug.log('SjzApi', '═══════════════════════════════════');
    Mode.Debug.log('SjzApi', '✅ API 层已加载（仅本地服务器直读）');
  }
  ...
  Mode.SjzApi = {
    ...
    // ── 🛡️ 卡战备（从本地服务器读取，V3格式，已过滤兑换组+排序取前10）──
